...
dataset1 = Dataset(r"datasets\dataset")
dataset2 = Dataset(r"datasets\dataset")

# Создаём объект класса SIFT для работы с двумя датасетами
sift = SIFT(dataset1, dataset2)

# Извлекаем дескрипторы и метки для каждого датасета
sift.extract_features(dataset1)
sift.extract_features(dataset2)
```

**Если необходимо сразу получить оба объекта изображений:**
...
similar_image = sift.find_similar_image(my_image, dataset2)

# Визуализируем оба изображения для визуальной оценки
my_image.visualize()
similar_image.visualize()
```

**Если необходимо только визуализировать изображения:**

```python
# Получаем объект изображения по названию
my_image = dataset1.get_image("2_2.jpg")

# Ищем схожее изображение и визуализируем его
...
**Для объединения всех методов поиска дубликатов и схожих изображений мы объеденили весь главный функционал в один класс VisDatCompare:**
...
1. Быстро отсеить дубликаты по exif-характеристикам
...
Переходим в нужную деректорию и клонируем репозиторий:
