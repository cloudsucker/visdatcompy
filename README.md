...
> [!WARNING] 
> Данный проект устарел, его работоспособность не гарантирована в связи с прекращением его поддержки.
...
> [!TIP]
> Pixel to pixel (pix2pix) не является полноправной метрикой, так как по сути своей это полное сравнение всех пикселей двух изображений между собой, однако для удобства был реализован в классе `Metrics`. Выходными данными этого метода является булево значение: `True` при полной схожести и `False` если есть различие. Его с лёгкостью заменяет [**сравнение по метаданным**](#exif).
